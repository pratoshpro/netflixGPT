...
import { addGPTResults, resetGPTState } from "@/utils/gptSlice";
...
const SearchBar = () => {
  ...
  const searchMovies = async () => {
    if (searchRef.current?.value) {
      const movieResults = await fetchMovie(searchRef.current!.value);
      dispatch(
        addGPTResults({
          movieNames: [searchRef.current!.value],
          movieResults,
        })
      );
    }
  ...
  return (
    <div className="pt-[30%] md:pt-[8%]">
      <form
        className="rounded-md w-fit md:w-1/2 bg-black bg-opacity-70 grid grid-cols-12 m-auto"
